...
import {
  MutationCreateTheaterArgs,
  MutationDeleteTheaterArgs,
  Query,
  Theater
} from "./gen-types";
import "./App.css";
...
const App: React.FC = () => {
  ...
  const [createTheater] = useMutation<MutationCreateTheaterArgs>(
    CREATE_THEATER
  );
  const [deleteTheater] = useMutation<MutationDeleteTheaterArgs>(
    DELETE_THEATER
  );
    ...
    <div className="app">
      ...
      >
        <input
          ...
          type="text"
          placeholder="Enter artistic director"
      ...
      <ul>
        {data.theaters.map((theater: Theater) => (
          <li key={theater.id}>
            <span className={theater.active ? "active" : "inactive"}>
              {theater.artisticDirector}
            </span>
            <button
              className="btn btn-sm btn-danger rounded-circle float-right"
              onClick={() => {
                deleteTheater({ variables: { id: theater.id } });
                window.location.reload();
              }}
            >
              X
